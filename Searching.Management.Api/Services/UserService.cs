...
public class UserService : BaseService
{
    ...
    public UserService(IUnitOfWork _unitOfWork, ITokenHelper tokenHelper, IOtpHelper otpHelper,
        IMailService mailService, IOptions<AppSettings> appSettings) : base(_unitOfWork)
    {
    ...
    public async Task<LoginResponse> LoginAsync(LoginRequest request)
    {
        ...
        if (user == null) throw new DomainBadRequestException("Invalid username or password");

        if (user.IsVerified == false) throw new DomainBadRequestException("Account is not verified");

        var passwordCheck = PasswordHelper.Verify(request.Password, user.Password);

        if (!passwordCheck) throw new DomainBadRequestException("Invalid username or password");
    ...
    public async Task<RegisterResponse> RegisterAsync(RegisterDto request)
    {
        ...
        try
        {
            ...
            if (send == null) throw new DomainExternalServiceException("Error sending email");
            return await Task.FromResult(new RegisterResponse
            {
    ...
    public async Task<RegisterResponse> RegisterWithOtp(RegisterDto request)
    {
        ...
        try
        {
            ...
            await _mailService.SendMail(savedUser.Email, "Account Activation", template);
    ...
    public async Task<VerifyResponse> VerifyAsync(string token)
    {
        ...
        if (!validity) throw new DomainBadRequestException("Invalid tokens");
        ...
        if (user == null) throw new DomainUnauthorizedException("Access denied");

        if (user.IsVerified) throw new DomainBadRequestException("Account already verified");
